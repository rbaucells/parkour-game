...
public class MultiRaycastShooting : MonoBehaviour
{
    ...
    public enum MultipleAimType
    {
        Opposite,
        Same
    }
    [SerializeField] MultipleAimType[] multipleAimType;
    ...
    void RaycastFire()
    {
        ...
        {
            Transform attackPoint = attackPoints[0];
            Ray ray = new();
            
            switch (multipleAimType[i])
            {
                case MultipleAimType.Opposite:
                    ray = new(attackPoint.position, (GetTargetPointOpposite() - attackPoint.position).normalized);
                    break;

                case MultipleAimType.Same:
                    ray = new(attackPoint.position, (GetTargetPointForward() - attackPoint.position).normalized);
                    break;
            }
            ...
            {
                StartCoroutine(RaycastTrail(attackPoint.position, GetTargetPointForward(), Vector3.zero, false));
            }
        }
    }
    

    Vector3 GetTargetPointForward()
    {
        // randominizer
    ...
    Vector3 GetTargetPointOpposite()
    {
        // randominizer
        Vector3 direction = Quaternion.AngleAxis(Random.Range(-bulletSpread.x, bulletSpread.x), aimingCameraContainer.up) * Quaternion.AngleAxis(Random.Range(-bulletSpread.y, bulletSpread.y), aimingCameraContainer.right) * aimingCameraContainer.forward;
